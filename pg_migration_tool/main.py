...
"""
    ...
    def generate_pg_dump_and_restore_cmd(self, event: Select.Changed)-> str:
        ...
        db = config["dbs"][event.value]
        dump_path = self.construct_path_to_dump(db)
        pg_dump_cmd = f'PGPASSWORD=\'{db['source']['db_password']}\' pg_dump -T \'*awsdms*\' -h {db['source']['db_connection_host']} -p {db['source'].get('port', 5432)} -U {db['source']['db_username']} -d {db['source']['db_database_name']} --create --clean --encoding utf8 --format directory --jobs {concurrency} -Z 0 -v --file={dump_path}'
